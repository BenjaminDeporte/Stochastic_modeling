...
def forward(X, weights):
    ...
    B_mu = weights["mu"]["B"]
    mu = X @ W_mu + B_mu
    W_logsig = weights["logsig"]["W"]
    B_logsig = weights["logsig"]["B"]
    logsig = X @ W_logsig + B_logsig
    
    return mu, logsig # (N, D_Y)

#-------------------------------------------------------------------
# probabilistic model declaration
#-------------------------------------------------------------------
...
def execute_main():
    ...
    sampled_weights["logsig"] = {"W":posterior_samples["W_logsig"], "B":posterior_samples["B_logsig"]}
    ...
    # use vmap to run predictions on each of the sampled neural nets
    mu_preds, logsig_preds = vmap(forward, in_axes=(None, in_axes_tree))(X_test, sampled_weights)
    sigma_preds = jnp.exp(logsig_preds)
    ...
    # y_low_ci = np.quantile(mu_preds, q=0.025, axis=0).squeeze()
    # y_high_ci = np.quantile(mu_preds, q=0.975, axis=0).squeeze()
    y_low_ci = (y_mean - 1.96 * np.mean(sigma_preds, axis=0)).squeeze()
